    ...
    for agent_uuid in agent_uuid_list:
        ...
        for dialog_info in data['result']:
            if 'status_scheme' in dialog_info and dialog_info['status_scheme'] == 'Отмена перехода':
                ...
                worksheet.write(f'C{str(change_count)}', get_name_queue(token, dialog_info['campaign_id']).split('t2_upload_')[1][:-5])
                worksheet.write(f'D{str(change_count)}', dialog_info['CallDateTime'])
